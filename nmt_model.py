...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        h_concatenated = torch.cat([h_forwards, h_backwards], dim = 1)  
        init_decoder_hidden = self.h_projection(h_concatenated) # h_0^{dec}

        c_forward, c_backward = last_cell[0], last_cell[1]  
        c_concatenated = torch.cat([c_forward, c_backward], dim=1)  
        init_decoder_cell = self.c_projection(c_concatenated) # c_0^{dec}
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 3. Use the torch.split function to iterate over the time dimension of Y.
        for Y_t in Y:
            Y_t = torch.squeeze(Y_t) # queeze Y_t from (1, b, e) to (b, e)
            Ybar_t = torch.cat([Y_t, o_prev], dim = 1)  # concatenate Y_t with o_prev on their last dimension -> (b, e + h)
            # compute the Decoder's next (cell, state) values as well as the new combined output o_t
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # dec_hidden is shape (b, h)， unsqueeze to (b, h, 1)
        # enc_hiddens_proj is shape (b, src_len, h)
        # mutiplication to shape (b, src_len, 1), squeeze to (b, src_len)
        ...
        # 1. Apply softmax to e_t to yield alpha_t
        alpha_t = torch.nn.functional.softmax(e_t, dim=1) # et: (b, src_len)
        alpha_t = alpha_t.unsqueeze(dim=1) # (b, src_len) to (b, 1, src_len)

        # 2. Use batched matrix multiplication between alpha_t and enc_hiddens to obtain the attention output vector, a_t.
        a_t = alpha_t.bmm(enc_hiddens)  # (b, 1, src_len) * (b, src_len, 2h) -> (b, 1, 2h)
        a_t = a_t.squeeze(dim=1)  # (b, 1, 2h) -> (b, 2h)

        # 3. Concatenate dec_hidden with a_t to compute tensor U_t
        U_t = torch.cat([a_t, dec_hidden], dim=1)  # (b, h) + (b, 2h) -> (b, 3h)
